...
describe('MobClearSystem', () => {
  it('rejects empty fields and insufficient gold without charging', () => {
    const clear = vi.fn(() => ({ clearedEnemies: 3, rewardGold: 27 }));
    const system = new MobClearSystem();
    expect(system.purchase(MOB_CLEAR_BASE_COST, 0, clear)).toEqual({ success: false, gold: MOB_CLEAR_BASE_COST, clearedEnemies: 0, rewardGold: 0 });
    expect(system.purchase(MOB_CLEAR_BASE_COST - 1, 3, clear)).toEqual({ success: false, gold: MOB_CLEAR_BASE_COST - 1, clearedEnemies: 0, rewardGold: 0 });
    expect(clear).not.toHaveBeenCalled();
  });

  it('charges once, clears the current wave, and raises the next price', () => {
    const system = new MobClearSystem();
    const clear = vi.fn(() => ({ clearedEnemies: 100, rewardGold: 1_800 }));
  ...
  it('does not charge when the clear callback reports no enemies', () => {
    const system = new MobClearSystem();
    expect(system.purchase(5_000, 2, () => ({ clearedEnemies: 0, rewardGold: 0 }))).toEqual({
      success: false, gold: 5_000, clearedEnemies: 0, rewardGold: 0,
    });
